...
import com.ecommerce.paymentservice.dto.PayPalCaptureResponse;
import com.ecommerce.paymentservice.dto.PayPalCaptureRequest;
...
public class PaymentService {
    ...
    public PayPalPaymentResponse initiatePayment(PaymentInitiateRequest request) {
        ...
        BigDecimal usdAmount = exchangeRateService.convertLkrToUsd(savedPayment.getAmount());
        ...
        if (payPalOrder.getLinks() != null) {
            for (PayPalCreateOrderResponse.PayPalLink link : payPalOrder.getLinks()) {
                if ("approve".equalsIgnoreCase(link.getRel()) || "payer-action".equalsIgnoreCase(link.getRel())) {
        ...
        response.setAmount(usdAmount);
        response.setOriginalAmountLkr(savedPayment.getAmount());
    ...
    public PayPalCreateOrderResponse createPayPalOrder(Payment payment) {
        ...
        BigDecimal usdAmount = exchangeRateService.convertLkrToUsd(payment.getAmount());

        Map<String, Object> amount = new HashMap<>();
        amount.put("currency_code", payPalConfig.getCurrency());
        amount.put("value", usdAmount.toString());
    ...
    public PayPalCaptureResponse capturePayPalOrder(String paypalOrderId) {
        ...
        PayPalCaptureResponse captureResponse = webClient.post()
                .uri(payPalConfig.getBaseUrl() + "/v2/checkout/orders/" + paypalOrderId + "/capture")
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken)
                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                .retrieve()
                .onStatus(
                        status -> status.isError(),
                        clientResponse -> clientResponse.bodyToMono(String.class)
                                .map(errorBody -> new RuntimeException("PayPal capture failed: " + errorBody))
                )
    ...
    public PaymentResponse completePayPalPayment(PayPalCaptureRequest request) {
        PayPalCaptureResponse captureResponse = capturePayPalOrder(request.getPaypalOrderId());
        ...
        successRequest.setOrderId(request.getOrderId());
        successRequest.setPaymentReference(captureResponse.getId());

        return markPaymentSuccess(successRequest);
    }
